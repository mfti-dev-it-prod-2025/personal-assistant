...
@pytest.mark.asyncio
async def test_get_category_by_id(router_api_user, test_category):
    resp = router_api_user.get(
        f"/api/v1/expense_category/?id={test_category['id']}"
    )

    assert resp.status_code == 200
    assert resp.json()["name"] == test_category["name"]


@pytest.mark.asyncio
async def test_get_category_by_name(router_api_user, test_category):
    resp = router_api_user.get(
        f"/api/v1/expense_category/?name={test_category['name']}"
    )
...
@pytest.mark.asyncio
async def test_get_all_categories(router_api_user, test_category):
...
def test_update_category(router_api_user, test_category):
    payload = {"description": "Обновленное описание"}

    resp = router_api_user.put(
        f"/api/v1/expense_category/{test_category['name']}",
        json=payload,
    )
...
@pytest.mark.asyncio
async def test_delete_category(router_api_user, test_category):
